...
function createIncomingRequestDebugMiddleware(options = {}) {
  ...
  return function incomingRequestDebugMiddleware(req, res, next) {
    ...
    const parsedRawBody = typeof req.rawBody === 'string' ? tryParseJson(req.rawBody) : undefined;
    ...
    const log = {
      ...
      // Prefer structured raw body (parsed JSON) so token redaction works.
      raw_body:
        typeof req.rawBody === 'string'
          ? sanitizeBody(parsedRawBody !== undefined ? parsedRawBody : req.rawBody, options.bodySanitizer)
          : undefined,
